  ...
  const fetchLeaderboard = async (selectedMode: Mode) => {
    ...
    try {
      ...
      const response = await fetch(
        `${process.env.API_URL ?? 'http://localhost:8000'}/api/users/leaderboard/?mode=${selectedMode}&limit=50`,
    ...
    <div className="flex flex-col md:flex-row h-[calc(100vh-130px)] gap-6 w-full max-w-[1800px] mx-auto p-4 relative font-['Outfit'] text-[1.1rem]">
      ...
      <div className="hidden lg:flex flex-col w-80 gap-3 shrink-0 overflow-hidden">
        <div
          ...
          style={{
            background: isLight
              ? 'linear-gradient(160deg, #fdf8f0 0%, #faf8f5 55%, #f8f7f4 100%)'
              : 'linear-gradient(160deg, #161208 0%, #0d0d0d 55%, #090909 100%)',
      ...
      <div className="hidden xl:flex flex-col w-96 gap-5 shrink-0 overflow-hidden">
        <div
          className="relative rounded-[36px] overflow-hidden shrink-0"
          style={{
            background: isLight ? 'linear-gradient(160deg, #ffffff 0%, #f8f8f8 100%)' : 'linear-gradient(160deg, #0f0f0f 0%, #0a0a0a 100%)',
            border: `1px solid rgba(${isLight ? '0,0,0' : '255,255,255'},${isLight ? '0.1' : '0.05'})`,
          }}
        >
