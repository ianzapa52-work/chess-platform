  ...
  return (
    <div className="bg-black/50 in-[.light]:bg-white h-full flex flex-col border border-white/7 in-[.light]:border-gray-200 rounded-[2.5rem] overflow-hidden shadow-2xl backdrop-blur-xl">
      ...
      <div className="px-6 py-5 border-b border-white/5 in-[.light]:border-gray-100 flex justify-between items-center">
        <div>
          <p className="text-[10px] font-black tracking-[0.4em] text-zinc-500 in-[.light]:text-gray-500 uppercase">
            Movimientos
          </p>
          <p className={`text-sm font-bold mt-1 transition-colors duration-200 ${
            gameStarted ? 'text-white/80 in-[.light]:text-gray-900' : 'text-zinc-600 in-[.light]:text-gray-400'
          }`}>
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            onClick={toggleChat}
            className="group relative w-12 h-12 rounded-2xl bg-white/10 in-[.light]:bg-gray-100 border border-white/20 in-[.light]:border-gray-300 hover:bg-white/20 in-[.light]:hover:bg-gray-200
          ...
          >
            <span className={`text-lg transition-all duration-200 group-hover:scale-110 ${
              chatOpen
                ...
                : 'text-zinc-400 in-[.light]:text-gray-500'
            }`}>
              💬
            </span>
      ...
      <div className="flex flex-col flex-1 overflow-hidden relative">
        ...
        <div
          ref={scrollRef}
          className="flex-1 overflow-y-auto p-5 space-y-2 custom-scrollbar bg-linear-to-b from-black/20 to-black/50 in-[.light]:from-gray-50 in-[.light]:to-gray-100/50 pr-2 min-h-0"
        >
          {rows.length === 0 ? (
        ...
        {/* Chat */}
        <div className={`
          transition-all duration-300 ease-[cubic-bezier(0.4,0,0.2,1)] overflow-hidden border-t border-white/8 in-[.light]:border-gray-100
          bg-linear-to-b from-zinc-900/30 to-black/50 in-[.light]:from-gray-50 in-[.light]:to-gray-100 backdrop-blur-xl
          ${chatOpen
        ...
        `}>
          {chatOpen && (
            <>
              <div ref={chatScrollRef} className="h-[230px] overflow-y-auto space-y-3 custom-scrollbar pr-2 p-5">
                {messages.map((msg, index) => (
                  <div key={`${index}-${msg}`} className="flex gap-3 break-words animate-in slide-in-from-bottom-2 duration-150">
                    <span className={`font-bold text-xs px-3 py-1.5 rounded-full uppercase tracking-wide min-w-[60px] text-center shrink-0 ${
                      msg.startsWith('Tú:')
                        ? 'bg-emerald-500/20 text-emerald-300 in-[.light]:text-emerald-700 border border-emerald-500/30 in-[.light]:border-emerald-400/40'
                        : msg.includes('🔴')
                        ? 'bg-red-500/20 text-red-300 in-[.light]:text-red-700 border border-red-500/30 animate-pulse'
                        : msg.includes('½')
                        ? 'bg-yellow-500/20 text-yellow-300 in-[.light]:text-yellow-700 border border-yellow-500/30 animate-pulse'
                        : 'bg-zinc-700/40 in-[.light]:bg-gray-200 text-zinc-300 in-[.light]:text-gray-600 border border-zinc-600/30 in-[.light]:border-gray-300'
                    }`}>
                      {msg.split(':')[0]}
                    </span>
                    <span className="text-sm leading-relaxed text-zinc-200 in-[.light]:text-gray-700 flex-1">
              ...
              <div className="px-5 py-3 bg-black/50 in-[.light]:bg-gray-100 backdrop-blur-sm border-t border-white/[0.15] in-[.light]:border-gray-200 shadow-2xl">
                <form onSubmit={sendMessage} className="flex gap-3">
                  <input
                    ...
                    className={`flex-1 bg-zinc-900/80 in-[.light]:bg-white border rounded-2xl px-4 py-3.5 text-sm
                              text-white in-[.light]:text-gray-900 placeholder-zinc-400 in-[.light]:placeholder-gray-400 focus:outline-none focus:border-emerald-400/80
                              focus:ring-2 focus:ring-emerald-400/30 transition-all duration-150 shadow-2xl
                              hover:border-zinc-500/80 in-[.light]:hover:border-gray-400 hover:shadow-zinc-900/30 hover:shadow-2xl
                              ${isProcessingCommand
                                ? 'opacity-60 cursor-not-allowed border-zinc-700/50 in-[.light]:border-gray-300'
                                : 'border-zinc-700/50 in-[.light]:border-gray-300'}`}
                    maxLength={120}
                    autoComplete="off"
                  ...
                  <button
                    ...
                    className="w-14 h-14 rounded-2xl bg-linear-to-r from-emerald-500/50 to-emerald-600/50
                ...
                {isProcessingCommand && (
                  <p className="text-[10px] text-yellow-400 in-[.light]:text-yellow-600 font-bold mt-1 text-center tracking-wider animate-pulse">
      ...
      {/* Banner de game over */}
      {gameState.isGameOver && (
        <div className={`px-6 py-4 border-t border-white/[0.12] in-[.light]:border-gray-100 shrink-0 relative z-10 ${
          gameState.isVictory
            ? 'bg-linear-to-r from-emerald-900/50 to-emerald-800/30 in-[.light]:from-emerald-50 in-[.light]:to-emerald-100/80 shadow-emerald-500/30'
            : 'bg-linear-to-r from-red-900/30 to-rose-900/20 in-[.light]:from-red-50 in-[.light]:to-rose-50'
        }`}>
          <p className={`text-center text-[11px] font-black uppercase tracking-[0.3em]
            px-4 py-2 rounded-2xl backdrop-blur-sm shadow-xl transition-all duration-300 ${
              gameState.isVictory
                ? 'bg-emerald-500/50 in-[.light]:bg-emerald-500 border-2 border-emerald-500/70 in-[.light]:border-emerald-600 text-emerald-50 scale-110 animate-pulse shadow-2xl shadow-emerald-500/60'
                : 'bg-red-500/30 in-[.light]:bg-red-100 border border-red-500/50 in-[.light]:border-red-300 text-red-200 in-[.light]:text-red-700'
            }`}>
            {status}
          </p>
        </div>
