  ...
  return (
    <>
      ...
      <aside
        ...
        className={`fixed top-0 right-0 h-full w-full max-w-[450px] bg-[#050505]/95 [.light_&]:bg-white border-l border-gold/20
          z-[999] shadow-[-20px_0_50px_rgba(0,0,0,0.9)] [.light_&]:shadow-[-20px_0_50px_rgba(0,0,0,0.08)] flex flex-col
          transition-transform duration-500 ease-in-out
          ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}
      >
        {/* Header */}
        <header className="p-6 border-b border-gold/10 bg-gradient-to-r from-zinc-900 via-black to-zinc-900 [.light_&]:from-white [.light_&]:via-gray-50 [.light_&]:to-white relative shrink-0">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              <div className="relative w-14 h-14 rounded-full bg-zinc-900 [.light_&]:bg-gray-100 border-2 border-gold/40 overflow-hidden shadow-2xl">
              ...
              <div>
                <h3 className="text-white [.light_&]:text-gray-900 font-serif font-medium text-2xl tracking-tight leading-tight italic">
                  {friendUsername ?? 'Chat'}
                </h3>
                <div className="flex items-center gap-2 mt-1">
                  <Shield size={12} className="text-gold" />
                  <span className="text-[10px] font-sans font-bold tracking-[0.2em] text-zinc-400 [.light_&]:text-zinc-500 uppercase">
                    {wsReady ? 'Conectado' : 'Conectando...'}
                  </span>
                </div>
              </div>
            </div>
            <button onClick={handleClose} className="group p-2 hover:bg-white/5 [.light_&]:hover:bg-gray-100 rounded-full transition-all cursor-pointer">
              <X size={24} className="text-zinc-500 [.light_&]:text-gray-400 group-hover:text-gold group-hover:rotate-90 transition-all duration-300" />
            </button>
          </div>
        </header>
        ...
        >
          {loadingRoom && (
            <div className="flex flex-col items-center justify-center h-full gap-4 text-zinc-600 [.light_&]:text-zinc-400">
          ...
          {!loadingRoom && !error && messages.length === 0 && (
            <div className="flex flex-col items-center justify-center h-full gap-3 text-zinc-700 [.light_&]:text-zinc-400">
          ...
          {!loadingRoom && messages.map((msg) => {
            ...
            return (
              <div key={msg.id} className={`flex flex-col ${mine ? 'items-end' : 'items-start'}`}>
                {!mine && (
                  <span className="text-[9px] text-zinc-600 [.light_&]:text-zinc-500 font-bold tracking-widest uppercase px-1 mb-1">
                    {msg.sender_username}
                  </span>
                )}
                <div className={`max-w-[85%] px-5 py-3.5 rounded-2xl text-[15px] font-sans leading-relaxed shadow-lg
                  ${mine
                    ...
                    : 'bg-zinc-900 [.light_&]:bg-gray-100 text-zinc-100 [.light_&]:text-gray-800 border border-white/5 [.light_&]:border-gray-200 rounded-tl-none'
                  }`}>
                  {msg.text}
                </div>
                <span className="text-[9px] mt-1.5 font-bold tracking-tighter text-zinc-600 [.light_&]:text-zinc-400 uppercase px-1">
        ...
        {/* Input */}
        <footer className="p-6 bg-zinc-950 [.light_&]:bg-white border-t border-gold/10 [.light_&]:border-gray-100 shadow-[0_-10px_30px_rgba(0,0,0,0.5)] [.light_&]:shadow-[0_-4px_20px_rgba(0,0,0,0.06)] shrink-0">
          <form
            onSubmit={handleSend}
            className="flex items-center gap-3 bg-white/5 [.light_&]:bg-gray-50 border border-white/10 [.light_&]:border-gray-200 p-2 pl-5 rounded-full focus-within:border-gold/40 focus-within:bg-white/10 [.light_&]:focus-within:bg-white transition-all"
          >
            <input
              value={inputValue}
              onChange={e => setInputValue(e.target.value)}
              disabled={!wsReady || loadingRoom}
              className="flex-grow bg-transparent outline-none text-white [.light_&]:text-gray-900 text-sm font-sans placeholder:text-zinc-600 [.light_&]:placeholder:text-zinc-400 disabled:opacity-40"
          ...
          <div className="mt-4 flex justify-center">
            <p className="text-[8px] text-zinc-600 [.light_&]:text-zinc-400 font-bold tracking-[0.3em] uppercase">
