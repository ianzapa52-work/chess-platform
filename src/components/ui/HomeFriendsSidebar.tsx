  ...
  const fetchFriends = useCallback(async () => {
    try {
      ...
      const resolved: Friend[] = profiles
        ...
        .map(({ value: v }) => ({ ...v, last_seen: v.last_seen ?? null }))
        .sort((a, b) => STATUS_RANK[friendStatus(a.last_seen)] - STATUS_RANK[friendStatus(b.last_seen)]);
        ...
        {loading ? (
          ...
          friends.map((f) => (
            <button
              key={f.id}
              onClick={() =>
                window.dispatchEvent(new CustomEvent('open-chat', { detail: { username: f.username } }))
              }
            ...
            >
              <div className="relative shrink-0">
                ...
                />
                {(() => {
                  const st = friendStatus(f.last_seen);
                  const cls = st === 'online' ? 'bg-green-400 shadow-[0_0_5px_rgba(74,222,128,0.8)]'
                    : st === 'away' ? 'bg-amber-400 shadow-[0_0_5px_rgba(251,191,36,0.8)]'
                    : 'bg-zinc-600';
                  return <div className={`absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full border-2 border-black in-[.light]:border-white ${cls}`} />;
                })()}
              </div>
